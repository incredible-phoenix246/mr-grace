...
const ModMobileSidebar = () => {
  ...
  return (
    <>
      ...
      <section
        className={cn(
          "py-6 md:hidden  min-[400px]:pl-4 pl-2 flex flex-col w-full max-w-[230px] min-[400px]:max-w-[270px]  items-start bg-yellow-main fixed left-0 top-0 z-[999] h-screen transition-all opacity-0 sidebar-scroll overflow-x-hidden group select-none ",
          ShowAdminSidebar
      ...
      >
        ...
        <ul className="flex flex-col gap-y-3 min-[400px]:gap-y-4 py-8 mt-6">
            ...
            <Link
              ...
              className={cn(
                ...
                activeLink === link.link
                  ? "bg-white-main text-yellow-main rounded outline-none"
                  : "hover:bg-black-main/10 focus-visible:bg-black/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-yellow-main"
              )}
              onClick={() => {
                setActiveLink(link.link);
                setShowAdminSidebar(false);
          ...
          {/* LogOut */}
          <form className="flex flex-col w-full gap-y-6 xl:gap-y-8 pt-4 items-center">
            <button
              role="button"
              tabIndex={0}
              onClick={() => router.push("/")}
